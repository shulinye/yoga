...
def routine(li : list, imbalance, playLast = True, **kwargs):
    ...
    try:
        for i in range(len(li)-1):
            ...
            current_pose.play(imbalance=imbalance, nextMove=next_pose, **kwargs)
    ...
    return li_copy[-1].play(imbalance=imbalance,**kwargs)
...
def main():
    ...
    imbalance = []
    pose = movesGraph['child'].play(time=min(15, total_time//60), imbalance=imbalance)
    try:
        #warmup
        while time.time() - start < max(45,total_time//15):
            pose = pose.play(imbalance=imbalance, extended=True, early=True) #start slower
        #get me to table:
        ...
        moves.unlinkWarmup(movesGraph, imbalance=imbalance)
        pose = pose.play(nextMove=movesGraph['plank'])
        ...
        while time.time() < (end-max(30, total_time//10)):
            pose = pose.play(imbalance=imbalance, extended=True)
